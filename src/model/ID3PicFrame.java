...
public ID3PicFrame(String mimeType, byte type, EncodedText description, byte[] data, String keyword, int bodysize, short flags){
...
public byte[] getBytes(){
	...
	byte[] complete = new byte[keyword.length + size.length + flags.length + data.length];
...
public byte[] getPicBytes(){
	...
	return bytes;
	/*byte[] mimeType = this.mimeType.getBytes();
	byte[] type = new byte[1];
	type[0] = this.type;
	byte[] des =this.description.toString().getBytes();
	//System.out.println("c1:"+des.length);
	byte[] complete = new byte[mimeType.length + type.length + des.length + this.data.length];
	for(int i = 0; i < complete.length; i++){
		if(i < mimeType.length)
			complete[i] = mimeType[i];
		if(i >= mimeType.length && i < mimeType.length+1)
			complete[i] = type[i-mimeType.length];
		if(i >= mimeType.length+1 && i < mimeType.length+1+des.length)
			complete[i] = des[i-mimeType.length-1];
		if(i >= mimeType.length+1+des.length)
			complete[i] = data[i-mimeType.length-des.length-1];
	}
	//System.out.println("l2: "+data.length);
	return complete;*/
